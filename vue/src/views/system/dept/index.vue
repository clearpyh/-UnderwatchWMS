  ...
  listDept(queryParams.value).then(response => {
    ...
    const rename = (list) => {
      ...
      return list.map(node => {
        const n = { ...node };
        if (typeof n.deptName === 'string' && map.has(n.deptName)) {
          n.deptName = map.get(n.deptName);
        }
...
function handleAdd(row) {
  ...
  listDept().then(response => {
    ...
    deptOptions.value = rename(proxy.handleTree(response.data, "deptId"));
  });
  if (row != undefined) {
