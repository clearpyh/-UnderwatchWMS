...
function initChart() {
  ...
  const avg = (props.chartData.yData && props.chartData.yData.length)
    ? props.chartData.yData.reduce((a, b) => a + b, 0) / props.chartData.yData.length
    : 0
  chart.value.setOption({
    tooltip: {
      ...
      formatter: props.tooltipFormatter
        ? (params) => props.tooltipFormatter(params, avg)
        : (params) => {
            const p = Array.isArray(params) ? params[0] : params
            const date = p.axisValue
            const val = p.data
            const over = val > avg ? '超过平均值' : '低于平均值'
            return `${date}<br/>订单量：${val}（${over}）`
          }
